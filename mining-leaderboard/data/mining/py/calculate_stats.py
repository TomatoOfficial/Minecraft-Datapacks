...
for i in range(file_count):
    ...
    try:
        with open(stat_filepath, 'r', encoding='utf-8') as f:
            ...
            try: # try decode JSON
                data = json.loads(stat_json)
            except json.JSONDecodeError as json_err:
                print(f"JSON Decode Error: {json_err}")
                print(f"File may corrupted: {stat_filepath}")
                print("\n")
                continue
            ...
            if "minecraft:custom" in data["stats"]:
                custom_data = data["stats"]["minecraft:custom"]
                
                # 获取行走和疾跑距离
                walk_distance = custom_data.get("minecraft:walk_one_cm", 0)
                sprint_distance = custom_data.get("minecraft:sprint_one_cm", 0)
                
                print(f"Walk Distance: {walk_distance}cm")
                print(f"Sprint Distance: {sprint_distance}cm")
                
                # 判断是否为BOT：行走和疾跑距离都小于100米
                if walk_distance < 10000 and sprint_distance < 10000:
                    is_bot = True
                    bot_reason = f"Walk Distance: {walk_distance}cm and Sprint Distance: {sprint_distance}cm are less than 10000cm"
                else:
                    # print(f"Real Player")
                    pass
            else:
                # 如果没有custom数据，无法判断，默认视为BOT或新玩家
                is_bot = True
                bot_reason = "minecraft:custom Data not found"
                print("minecraft:custom Data not found")

            if is_bot:
                print(f"Judgment as a BOT: {bot_reason}")
                print("\n")
                continue
            
            # 如果不是BOT，继续统计挖掘次数
            if "minecraft:mined" not in data["stats"]:
                print("minecraft:mined Data not found")
                print("\n")
                continue

            stat_data = data["stats"]["minecraft:mined"]

            def calculate_total_mining(stat_data, id_list):
                total_mining = 0
                for block_id in id_list:
                    key = f"minecraft:{block_id}"
                    if key in stat_data:
                        total_mining += stat_data[key]
                return total_mining

            total_usage = calculate_total_mining(stat_data, block_list)

            print(f"总数: {total_usage}")
            print("\n")

            calculation_time = time.strftime("%Y%m%d-%H%M%S", time.localtime()) 
            function_save_filename = player_name.lower() + "_saved_" + calculation_time + ".mcfunction"
            function_save_filepath = os.path.join(directory_path, function_save_filename)

            with open(function_save_filepath, "w", encoding='utf-8') as save_file:
                # 保存实际数据
                save_file.write(f"# 玩家: {player_name}\n")
                save_file.write(f"# UUID: {player_uuid}\n")
                save_file.write(f"# 行走距离: {walk_distance}cm\n")
                save_file.write(f"# 疾跑距离: {sprint_distance}cm\n")
                #save_file.write(f"# 工具使用总次数: {total_usage}\n")
                #save_file.write(f"# 统计时间: {calculation_time}\n")
                save_file.write(f"scoreboard players set {player_name} ml.total {total_usage}\n")
            
            print(f"结果已保存到: {function_save_filename}")

            
    except Exception as e:
        print(f"Unknown Error: {e}")
        print(f"Processing File: {stat_filepath}")
os.system( 'pause ')
